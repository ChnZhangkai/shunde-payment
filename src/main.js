...
// 全局使用的统一表单校验
import $VALID from './api/checkUtil.js'
...
//根据路由判断操作是前进还是后退，并把标记值存入vuex
    ...
    // 当经营诊断报表页面返回上一页时,因为已经跳转到报表页面，说明已经登录过了，则直接返回到首页
    if (from.path === RouterManager.chartAnalyse && history.getItem('UPDATE_DIRECTION') === 'reverse') {
      router.push({
        name: '/main',
        params: {
          selected: '首页'
