...
class proveUtil {
  ...
  __initDefaults() {
    this.defaults = {
      messages: {
        ...
        deptname: this.formatTpl('请输入正确的单位名称'),
        agree: this.formatTpl('请阅读并同意条款'),
      }
    }
  }
  ...
  __initMethods() {
    ...
    that.methods = {
       ...
       */
      china(value) {
        return that.optional(value) || /^[\u4e00-\u9fa5]{2,6}$/.test(value)
      },
      /**
       * 校验营业执照，有15位、有18位、有全部数字，也有数字+字母，我干脆就校验它不能输入特殊字符和中文
       * 营业执照为13-18位非汉字及非特殊符号组成
       */
      licen(value) {
        let tempCheinese = /^[^\u4e00-\u9fa5]{0,}$/;
        let tempSpecial = /[`~-￥&=|!@#\$%\^\&\*\(\)_\+<>\?:"\{\},\.\\\/;'\[\]]/im;//这个都没有起作用
        if (!tempCheinese.test(value) || tempSpecial.test(value) || value.length < 13) {
          //不能输入汉字，不能输入特殊字符，输入长度必须大于13
          return false//输入内容有误
        }
        return true
      },
       ...
       * 验证单位名称
       */
      deptname(value, param) {
        return that.optional(value) || /^[A-Za-z0-9_()（）\-\u4e00-\u9fa5]+$/.test(value)
      },
      agree(value, param) {
