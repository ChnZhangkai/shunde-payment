...
function check18IdCard (idcard) {
  if (idcard != null && idcard != '') {
    let tempIdcard = /^[1-9]\d{5}[1-9]\d{3}((0\d)|(1[0-2]))(([0|1|2]\d)|3[0-1])\d{3}([0-9]|X)$/
    if (!tempIdcard.test(idcard)) {
      return false
    }
    return true
...
function checkLicense(licen) {
  if (licen != null && licen != '') {
    let reg = /^[0-9a-zA-Z]{13,18}$/
    return reg.test(licen)
  }
 ...
 * @param {String} amt 
 */
function checkRmbAmt (amt) {
  if (!amt) {
    return false
  }
  let reg = /^[0-9]([0-9]{1,9})?(\.[0-9]{1,2})?[\u4E07][\u5143]$/
  if (reg.test(amt)) {
    return true
  } else {
    return false
...
function checkCarNo (carNo) {
  ...
  // 只大致校验下，不用区分新能源车牌与普通车牌，所以按照最大长度校验
...
function generateSignTime() {
  ...
  return [year, month, day, hour, minute, second, msecond].map(formatNumber).join('')
}
