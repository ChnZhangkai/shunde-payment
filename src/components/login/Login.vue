  ...
  <div class="bgc">
    ...
    <div class="content">
      ...
      <div class="flex_space">
        ...
        <img
          v-show="!ifSetDisplay"
          @click="ifSetDisplay=!ifSetDisplay"
          class="eye"
          src="/static/assets/eye_close.png"
        >
        <img
          v-show="ifSetDisplay"
          @click="ifSetDisplay=!ifSetDisplay"
          class="eye"
          src="/static/assets/eye_open.png"
        >
      </div>
      <Divider></Divider>
...
<script>
  ...
  export default {
    ...
    mounted() {
      ...
      let that = this;
//    let cacheUserPhone = that.$CACHE.getCache("inputPhone");
      let cacheUserPhone = sessionStorage.getItem("inputPhone");
      //如果登陆的手机号有缓存，显示出来
      if (!that.$StringUtils.isEmpty(cacheUserPhone)) {
        that.vUserphone = cacheUserPhone;
      ...
      if (window.requestIdleCallback) {
        requestIdleCallback(function () {
          Fingerprint2.get(function (components) {
            let vals = components.map((comp) => {
              return comp.value
            })
            let murmur = Fingerprint2.x64hash128(vals.join(''), 31)
            // 把设备指纹存入sessionStorage中后续使用
...
//        that.$CACHE.putCache("cacheInputPhone", userPhoneInput);//提供给忘记密码用
      ...
      showResetPwd() {
        this.$dialog.alert(
          "   ",
          that.$ERRCODE.STATIC_ERRORCDDE.LOGIN_PWD_ERROR_5
        );
      },
      ...
      toForgetPwd() {
        ...
        this.$router.push({
          name: this.$RM.ForgetPwd,
          params: {phone: this.vUserphone}
        });
      },
      toFinger() {
        let that = this
        ...
        // 1.是否是设置手势密码校验
        if (this.$StringUtils.isEmpty(this.$StringUtils.getFingerLockPwd(this.vUserphone))){
          this.$toast("你未设置手势密码，请用手机账号密码登录");
          return;
        }

        // 2.密码输入错误5次,用户锁定
        if(sessionStorage.getItem('LoginErrorNum5')){
          this.$toast("密码输入错误5次，用户已锁定。您可24小时后重新输入登录或者通过忘记密码重新设置密码。");
          return;
        }

        // 3.手势密码是否关闭
        let mobilePhone = this.vUserphone
        let fingerKey = mobilePhone + "fingerLockType";
        if(localStorage.getItem(fingerKey) === '1'){
          this.$toast(
            "您已关闭手势密码登录，请密码登陆后在我的—安全设置中打开手势密码"
          );
          return;
        }
      ...
      toLogin() {
        ...
        var userPhone = that.$refs.rUserPhone.value;// 获取手机号

        if (!that.$StringUtils.isEmpty(userPhone)) {
//          that.$CACHE.putCache("inputPhone", userPhone);//存15分钟缓存

          sessionStorage.setItem("inputPhone", userPhone);//存15分钟缓存
        }
        ...
        console.log(
          "sign>>>" + sign + "、timestamp>>>" + timestamp + ">>>reqId>>>" + reqId
        );
        //开始请求登录接口
        ...
        that.$http
          .post(url, {
            ...
            data: {
              ...
              password: encStr,
              operType: "1",
              mac: sessionStorage.getItem('fingerprint') // 设备指纹编码
            } //operType操作类型0.注册，1.登陆
          })
          .then(function (res) {
            console.log(res);
            if (res.data.ret === "0") {
              ...
              //保留个登陆状态
              that.$CACHE.putCache("loginSession", token);//登陆态存个15分钟
              console.info("获取的token>>>" + token);

              that.$storeData.setSession(token);
              sessionStorage.setItem("cachePhone", userPhone);//手机号存个15分钟
//            that.$CACHE.putCache("cachePhone", userPhone);//手机号存个15分钟

              if (window.appId === "IOS" || window.appId === "Andriod") {
                window.interfacePhone.saveSession(
                  JSON.stringify({phone: userPhone, token: token})
                );
              }
              //跳转到首页
              console.log("是否第一次登录");
      ...
      getOpenApp() {
        //          跳转登陆失效的页面
        if (
          !this.$StringUtils.isEmpty(this.$StringUtils.getFingerLockPwd()) &&
          this.$StringUtils.getFingerLockType() == "2"
        ) {
      ...
      doLoginError(res, that) {
        if (res.data.data && res.data.data.errorTime) {
          ...
          console.warn("账号密码不对!!!");
          if (errorTime == 5) {
            sessionStorage.setItem('LoginErrorNum5', true)
            this.$toast("密码输入错误5次，用户已锁定。您可24小时后重新输入登录或者通过忘记密码重新设置密码。");

            // 密码输入错误5次，关闭手势密码开关

            // 以下为旧逻辑,不合理,暂时注释 2019-3-29 11:05
            ...
            this.$toast("密码输入错误，请重新输入。您还可以输入" + (5 - errorTime) + "次");
          }
        }
      }
    },
...
<style scoped>
  ...
  .img {
    ...
    height: 2.4rem;
    margin: auto;
    box-shadow: 2px 2px 10px 4px #FFE5D5, -2px -2px 10px 4px #FFE5D5;
    border-radius: .3rem;
  }
