    ...
    <div class="content">
      ...
      <div class='top'>
        <input class='inputPwd' type="tel" ref="rUserPhone" v-model="vUserPhone"
               placeholder="请输入手机号码" oninput="if(value.length>11)value=value.slice(0,11)"/>
        <img class='clearCss' v-show="vUserPhone" @click="clearPhone()" src="/static/assets/clear.png"/>
      </div>
      <!--横线-->
      <Divider></Divider>
      <div class='top'>
        <input class='inputPwd' type="text" ref="rSetResetPwd" v-model="vSetResetPwd"
               placeholder="请重置登录密码" v-if="ifResetDisplay" oninput="if(value.length>12)value=value.slice(0,12)"/>
        <input class='inputPwd' type="password" ref="rSetResetPwd" v-model="vSetResetPwd"
               placeholder="请重置登录密码" v-else oninput="if(value.length>12)value=value.slice(0,12)"/>

        <img class='clearCss' v-show="vSetResetPwd" @click="clearResetPwd()" src="/static/assets/clear.png"/>

        <img v-show="!ifResetDisplay" @click="ifResetDisplay=!ifResetDisplay" class="eye" src="/static/assets/eye_close.png"/>
        <img v-show="ifResetDisplay" @click="ifResetDisplay=!ifResetDisplay" class="eye" src="/static/assets/eye_open.png"/>
      </div>
      <!--横线-->
      <Divider></Divider>
      <div class='top'>
        <input class='inputPwd' type="text" ref="rSetPwd" v-model="vSetPwd"
               placeholder="请确认登录密码" v-if="ifDisplay" oninput="if(value.length>12)value=value.slice(0,12)"/>
      ...
      <div class='top'>
        <input class='inputPwd2' type="number" ref="rYanCode" v-model="vYanCode"
      ...
      <div class="redTips " v-show="checkTip">2次密码输入不一致</div>
  ...
  export default {
    ...
    data () {
      return {
        ...
        vSetResetPwd: null,//重置密码输入框的值
        ifSetDisplay: false,//显示隐藏
        ifDisplay: false,//显示隐藏
        ifResetDisplay: false,//显示隐藏
    ...
    methods: {
      ...
      isBtnEnable(){
        if (this.$utils.checkPwd(this.vSetPwd) && this.$utils.checkPwd(this.vSetResetPwd) && !this.$StringUtils.isEmpty(this.vUserPhone)
          && !this.$StringUtils.isEmpty(this.vYanCode) && !this.checkTip) {
...
//          跳转登陆失效的页面
    ...
    watch: {
      ...
      vSetPwd: {
        handler(){
          if (!this.$utils.checkPwd(this.vSetPwd)) {
            ...
            if(this.$utils.checkPwd(this.vSetResetPwd) && this.vSetPwd != this.vSetResetPwd){
              this.rexTip = false
              this.checkTip = true
            }else{
              this.rexTip = false
      ...
      vSetResetPwd: {
        handler(){
          if (!this.$utils.checkPwd(this.vSetResetPwd)) {
            this.checkTip = false
            this.rexTip = true//提示语出现
          } else {
            if(this.$utils.checkPwd(this.vSetPwd) && this.vSetPwd != this.vSetResetPwd){
              this.rexTip = false
              this.checkTip = true
            }else{
              this.rexTip = false
