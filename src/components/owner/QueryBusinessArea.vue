...
<template>
  <div class="QueryBusinessArea">
    ...
    <div class="edit_bg">
      ...
      <input :placeholder="place_holder" class="edit_text" v-model="creditNo" oninput="if(value.length>12)value=value.slice(0,12)" onkeyup="value=value.replace(/[^\d]/g,'')">
    </div>
    <Divider></Divider>
    <div class="page_rest">
...
<script>
  ...
  export default {
    ...
    data () {
      return {
        tips: '本次查询，要求客户输入对应烟草许可证编号，仅验证客户所在区域是否已开展业务。',
        edit_name: '烟草许可证编号：',
        place_holder: '请输入正确的烟草许可证编号',
    ...
    watch: {
      creditNo: function (val, oldVal) {
        if (val != null && val != undefined && val.length > 0 && val.length <= 12) {
          this.isInputNonEmpty = true
        } else if(val.length > 12){
          this.$toast('烟草许可证号不能大于12位数')
        }else {
        ...
        this.$api.post(this.$Constants.IS_AREA_OPEN, {
          ...
          if (result && result.data) {
            if (result.data.businessOpenFlag == "1") {
              //已开展业务跳转到首页
              this.$dialog.confirm('点击立即申请即可前往审核授信', '恭喜您！该区域业务已开展!', {confirmButtonText: '立即申请'})
                .then(function (action) {
                  ...
                  this.$router.push({name: this.$RM.Main, params: {selected: '首页'}})
                }.bind(this));
            } else {
              //未开展业务
